...
if [ -e "/usr/local/predict/requirements.txt" ]; then
    echo "Installing Python dependencies"
    $(which pip) install --user -r /usr/local/predict/requirements.txt
fi

# Install Predict as an editable package
if [ -e "/usr/local/predict/setup.py" ]; then
    echo "Installing Predict"
    $(which pip) install --user -e /usr/local/predict/
fi
...
if [ "$1" = "webserver" ] ; then
  i=0
  while ! ncat $POSTGRES_HOST $POSTGRES_PORT >/dev/null 2>&1 < /dev/null; do
    i=$((i+1))
    if [ "$1" = "webserver" ]; then
